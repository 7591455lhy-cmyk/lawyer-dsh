/**
 * lawyer-wizard 构建配置：Node 半（ESM）+ 浏览器 client bundle。
 * 结构与 lawyer-sidebar 完全一致（闭包工厂产物 + 模块表基线外部依赖），
 * 仅 PLUGIN_ID 不同；详细注释见 lawyer-sidebar/tsdown.config.mjs。
 *
 *   pnpm --dir <deepseek-harness 仓库路径> exec tsdown --config <本文件绝对路径>
 */
import { fileURLToPath } from 'node:url'

const PLUGIN_ID = 'lawyer-wizard'
