# lawyer-dsh Host 冒烟（可复跑）：lawyerFiles.save 子路径清洗与真实落盘。
# 用法：powershell -ExecutionPolicy Bypass -File smoke-save.ps1 [-Harness <dsh 源码根目录>]
param(
...
$entry = Join-Path $plugin 'smoke-save-entry.ts'
$out = Join-Path $plugin '.smoke-save.mjs'
